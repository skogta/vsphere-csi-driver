...
func TestRemoveFinalizerFromPVCWithUsedByAnnotations(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedFinalizers: []string{cnsoptypes.CNSPvcFinalizer},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify the PVC state - get the latest version
			updatedPVC := &v1.PersistentVolumeClaim{}
			err = fakeClient.Get(ctx, client.ObjectKey{
				Name:      pvc.Name,
			...
			if tt.shouldSkipRemoval {
				...
				// For cases where finalizer should be removed, we expect the operation to attempt removal
				// The actual removal behavior depends on the updateSVPVC implementation
				// Since this is a unit test of removeFinalizerFromPVC's logic, we mainly verify
				// that it doesn't return an error and properly handles the used-by annotation check
